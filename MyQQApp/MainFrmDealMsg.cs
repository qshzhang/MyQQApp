    ...
    partial class MainFrm
    {
        ...
        private void ReceiveMsgDeal(LocalMessage local)
        {
            ...
            if (-1 != index)
            {
                ...
                {
                    WaitRecvFileTimer.Elapsed += new ElapsedEventHandler((s, e) => OnTimedEvent(s, e, local.FriendAddr.IP, local.FriendAddr.TcpPort));
        ...
        private void RejectRecvFile(LocalMessage local)
        {
            ...
            tcpMessage.ClientSendMsgRecvFile(local.FriendAddr.IP, local.FriendAddr.TcpPort, GlobalValue.ResponseType.CancelRecv);
        }
        ...
        private void AcceptRecvFile(LocalMessage local)
        {
            ...
            if (saveFileDialog.ShowDialog(this) == DialogResult.OK)
            {
                ...
                tcpMessage.ClientStartMonitor(local.FriendAddr.IP,
                    local.FriendAddr.TcpPort, saveFileDialog.FileName, fileLen);

            }
            else
            {
                tcpMessage.ClientSendMsgRecvFile(local.FriendAddr.IP,
