...
namespace MyQQApp.ChatListControl
{
    ...
    public class ChatListItem
    {
        ...
        public Boolean GroupHeadContained(Point point)
        {
            if (!bounds.Contains(point)) return false;
            return !IsOpen || (SubItems.Count != 0 && point.Y > bounds.Y && point.Y < SubItems[0].Bounds.Y);
        }
        ...
        public int GroupContainAccount(string account)
        {
            for(int i = 0;i < SubItems.Count;i++)
            {
                if(account.CompareTo(SubItems[i].ID) == 0)
                {
                    return i;
                }
            }
        ...
        public class ChatListSubItemCollection : IList, ICollection, IEnumerable
        {
            ...
            public int GetOnLineNumber() {
                ...
                for (int i = 0, len = this.count; i < len; i++) {
                    if (m_arrSubItems[i].Status != UserStatus.OffLine)
                        num++;
                }
                return num;
            }
