...
namespace MyQQApp.ChatListControl
{
    public partial class ChatListBox : Control
    {
        ...
        private const int itemSmallHeight = 25;
        private const int itemBigHeight = 50;
        ...
        public ChatListBox()
        {
            ...
            this.itemColor = Color.SteelBlue;
            this.subItemColor = Color.SteelBlue;
        ...
        private ChatListItem selectGroup;
        /// <summary>
        /// 当前选中的组
        /// </summary>
        [Browsable(false)]
        public ChatListItem SelectGroup
        {
        ...
        public event ChatListEventHandler RightClickSubItem;
        public event ChatListEventHandler MouseEnterHead;
        public event ChatListEventHandler MouseLeaveHead;

        public delegate string GetNewPersonalMsg(string account);
        public event GetNewPersonalMsg GetNewPersonalMsgCallback;

        public delegate void ChatGroupEventHandler(object sender, ChatGroupEventArgs e);
        public event ChatGroupEventHandler RightClickItem;

        public delegate void ChatListBoxNullClickHander();
        public event ChatListBoxNullClickHander RightClickGenGroup;

        protected virtual void OnRightClickNullGenGroup()
        {
            if (this.RightClickGenGroup != null)
                RightClickGenGroup();
        }
        ...
        protected virtual void OnRightClickSubItem(ChatListEventArgs e)
        {
            if (this.RightClickSubItem != null)
                RightClickSubItem(this, e);
        }

        protected string OnLeftClickSubItem(string account)
        {
            if (this.GetNewPersonalMsgCallback != null)
               return GetNewPersonalMsgCallback(account);
            return "";
        }

        protected virtual void OnRightClickItem(ChatGroupEventArgs e)
        {
            if (this.RightClickItem != null)
                RightClickItem(this, e);
        }
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            Rectangle rectItem = new Rectangle(0, itemGap, this.Width, itemSmallHeight);                       //列表项区域
            Rectangle rectSubItem = new Rectangle(0, itemSmallHeight + itemGap, this.Width, (int)iconSizeMode);    //子项区域
            ...
            try
            {
                for (int i = 0, lenItem = items.Count; i < lenItem; i++)
                {
                    ...
                    if (items[i].IsOpen)
                    {                      //如果列表项展开绘制子项
                        rectSubItem.Y = rectItem.Bottom + itemGap;
                        for (int j = 0, lenSubItem = items[i].SubItems.Count; j < lenSubItem; j++)
                        {
                            DrawSubItem(g, items[i].SubItems[j], ref rectSubItem, sb);  //绘制子项
                            rectSubItem.Y = rectSubItem.Bottom + itemGap;             //计算下一个子项的区域
                            rectSubItem.Height = (int)iconSizeMode;
                        }
                        rectItem.Height = rectSubItem.Bottom - rectItem.Top - (int)iconSizeMode - itemGap;
                    }
                    items[i].Bounds = new Rectangle(rectItem.Location, rectItem.Size);
                    rectItem.Y = rectItem.Bottom + itemGap;           //计算下一个列表项区域
                    rectItem.Height = itemSmallHeight;
                }
                g.ResetTransform();             //重置坐标系
                chatVScroll.VirtualHeight = rectItem.Bottom - (itemSmallHeight + itemGap);   //绘制完成计算虚拟高度决定是否绘制滚动条
        ...
        {
            Boolean IsMouseRightButtonClicked = (((MouseEventArgs)e).Button == MouseButtons.Right);
            ...
            for(int i = 0;i < items.Count;i++)
            {
                ...
                {
                    selectGroup = items[i];
                    if (IsMouseRightButtonClicked)
                    {
                        if (items[i].GroupHeadContained(m_ptMousePos))
                        {
                            OnRightClickItem(new ChatGroupEventArgs(items[i]));
                            return;
                        }
                    }
                    ...
                    if (items[i].IsOpen)
                    {
                        for (int j = 0; j < items[i].SubItems.Count;j++)// ChatListSubItem subItem in item.SubItems)
                        {
                            if (items[i].SubItems[j].Bounds.Contains(m_ptMousePos))
                            {
                                if (items[i].SubItems[j].Equals(selectSubItem) && !IsMouseRightButtonClicked)
                                    return;

                                selectSubItem = items[i].SubItems[j];
                                items[i].SubItems[j].PersonalMsg = OnLeftClickSubItem(items[i].SubItems[j].ID);
                                this.Invalidate();
                                if (IsMouseRightButtonClicked)
                                {
                                    OnRightClickSubItem(new ChatListEventArgs(this.m_mouseOnSubItem, this.selectSubItem));
                                }
                                return;
                            }
                        }
                        if (new Rectangle(0, items[i].Bounds.Top, this.Width, 20).Contains(m_ptMousePos))
                        {
                            selectSubItem = null;
            ...
            if(true == IsMouseRightButtonClicked)
            {
                OnRightClickNullGenGroup();
            }
            ...
            //        if(IsMouseRightButtonClicked)
            //        {
            //            if(item.GroupHeadContained(m_ptMousePos))
            //            {
            //                OnRightClickItem(new ChatGroupEventArgs(item));
            //                return;
            //            }
            //        }
            ...
            //                {
            //                    if (subItem.Equals(selectSubItem) && !IsMouseRightButtonClicked)
            //                        return;

            //                    selectSubItem = subItem;
            //                    subItem.PersonalMsg = OnLeftClickSubItem(subItem.ID);
            //                    this.Invalidate();
            //                    if (IsMouseRightButtonClicked)
            //                    {
            //                        OnRightClickSubItem(new ChatListEventArgs(this.m_mouseOnSubItem, this.selectSubItem));
        ...
        protected virtual void DrawHeadImage(Graphics g, ChatListSubItem subItem, Rectangle rectSubItem)
        {
            ...
            if (subItem.HeadImage == null)                 //如果头像位空 用默认资源给定的头像
                subItem.HeadImage = global::MyQQApp.Properties.Resources._null;
            if (subItem.Status == UserStatus.OffLine)
                g.DrawImage(subItem.GetDarkImage(), subItem.HeadRect);
            else
            {
                g.DrawImage(subItem.HeadImage, subItem.HeadRect);       //如果在线根据在想状态绘制小图标
                if (subItem.Status == UserStatus.QMe)
                    g.DrawImage(global::MyQQApp.Properties.Resources.QMe, new Rectangle(subItem.HeadRect.Right - 10, subItem.HeadRect.Bottom - 10, 11, 11));
                if (subItem.Status == UserStatus.Away)
                    g.DrawImage(global::MyQQApp.Properties.Resources.Away, new Rectangle(subItem.HeadRect.Right - 10, subItem.HeadRect.Bottom - 10, 11, 11));
                if (subItem.Status == UserStatus.Busy)
                    g.DrawImage(global::MyQQApp.Properties.Resources.Busy, new Rectangle(subItem.HeadRect.Right - 10, subItem.HeadRect.Bottom - 10, 11, 11));
                if (subItem.Status == UserStatus.DontDisturb)
                    g.DrawImage(global::MyQQApp.Properties.Resources.Dont_Disturb, new Rectangle(subItem.HeadRect.Right - 10, subItem.HeadRect.Bottom - 10, 11, 11));
            }
        ...
        protected virtual void DrawLargeSubItem(Graphics g, ChatListSubItem subItem, Rectangle rectSubItem)
        {
            ...
            string strDraw = subItem.DisplayName.Trim();
            ...
            {                            //如果没有备注名称 这直接绘制昵称
                g.DrawString(subItem.NicName.Trim(), this.Font, Brushes.Black, rectSubItem.Height, rectSubItem.Top + 5);
            }           //绘制个人签名
        ...
        protected virtual void DrawSmallSubItem(Graphics g, ChatListSubItem subItem, Rectangle rectSubItem)
        {
            ...
            Size szFont = TextRenderer.MeasureText(strDraw.Trim(), this.Font);
        ...
        public ChatListSubItem[] GetSubItemsById(string userId)
        {
